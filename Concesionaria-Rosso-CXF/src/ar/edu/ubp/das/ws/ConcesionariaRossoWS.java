...
public class ConcesionariaRossoWS {
	...
	public String getClientes(@WebParam(name = "id_portal") String idPortal) throws Exception {
		...
		if (idPortal.equals(idPortalAprobado)) {
			...
			String idConcesionaria = "Rosso123";
	...
	public String notificarGanador(@WebParam(name = "id_portal") String idPortal,
		...
		if(idPortal.equals(idPortalAprobado)){
			...
			try {
				...
				/*
				 * Si el ganador es un cliente de esta concesionaria, actualiza
				 * valores en la tabla Clientes
				 */
				if (idConcesionaria.contains("Rosso")) {
					dao.update(adquirido);
					mensajeRespuesta = "Se ha cancelado la cuenta del cliente ganador del sorteo";
				} else {
					/*
					 * Si el ganador es un cliente de otra concesionaria, crea una
					 * entrada en la tabla Novedades
					 */
					String novedad = "El ganador del sorteo de la fecha " + fechaSorteo + " es " + dniCliente
							+ " de la concesionaria " + idConcesionaria;
